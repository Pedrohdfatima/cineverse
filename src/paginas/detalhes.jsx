...
export default function Detalhes() {
  ...
  // Estas variáveis dependem de 'dados', então as calculamos depois que 'dados' for carregado
  ...
  useEffect(() => {
    // Reseta os dados ao navegar para um novo item
    setDados(null);
    setRecommendations([]);
    ...
    const buscarDetalhes = async () => {
      try {
        const [detalhesRes, providersRes, recommendationsRes] = await Promise.all([
          ...
          axios.get(`https://api.themoviedb.org/3/${tipo}/${id}/watch/providers`, {
            params: {
              api_key: process.env.REACT_APP_TMDB_API_KEY,
            },
          }),
          // CORREÇÃO 1: URL correta -> "recommendations"
        ...
        // CORREÇÃO 2: Acessar "results" (plural)
        setRecommendations(recommendationsRes.data.results);
      } catch (err) {
        console.error("Erro ao buscar detalhes ou recomendações:", err);
      }
    };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.info}>
          ...
          {/* CORREÇÃO 3: Estrutura JSX corrigida e centralizada */}
          ...
          {dados.genres && (
            <p><strong>Gêneros:</strong> {dados.genres.map((g) => g.name).join(", ")}</p>
          )}
          {dados.runtime && (
            <p><strong>Duração:</strong> {dados.runtime} min</p>
          )}
          {dados.number_of_seasons && (
            <p><strong>Temporadas:</strong> {dados.number_of_seasons}</p>
          )}
          <p><strong>Lançamento:</strong> {new Date(dados.release_date || dados.first_air_date).toLocaleDateString("pt-BR")}</p>
          <p><strong>Nota:</strong> ⭐ {dados.vote_average?.toFixed(1)} ({dados.vote_count} votos)</p>
          {dados.homepage && (
            <p><a href={dados.homepage} target="_blank" rel="noopener noreferrer" className={styles.homepageLink}>Página oficial</a></p>
          )}
        </div>
      </div>
      ...
      {/* Seção Onde Assistir (sem alterações) */}
      <div>
        {/* ... seu código para "Onde Assistir" ... */}
      </div>
